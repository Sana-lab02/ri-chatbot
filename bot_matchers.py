...
from bot_utils import clean_text, safe_print
...
def find_best_row(user_input, df_customer_info, threshold=0.80):
    ...
    try:
        ...
        for size in range(1, len(words)+ 1):
            for i in range(len(words) - size + 1):
                ...
                if match:
                    retailer_candidate, score = match[0],match[1]
                    if score > best_score:
                        best_score = score
                        best_retailer = retailer_candidate
            print(f"Debug.. Best retailer match: '{best_retailer} {best_score}")


        if not best_retailer or best_score < threshold:
            print(f"(debug) Weak match '{best_retailer}' ({best_score}).")
            return None, None, best_score
        
        row_index = df_customer_info.index[df_customer_info["Retailer"] == best_retailer][0]
